...
export const ContactsWebScreen: React.FC = () => {
  ...
  const searchContacts = (query: string) => {
    return contacts.filter(contact => 
      ...
      (contact.prenom && contact.prenom.toLowerCase().includes(query.toLowerCase())) ||
      contact.telephone.includes(query)
    );
  };

  // Fonction de filtrage par statut Bob
  const filterByBobStatus = (hasBob: boolean) => {
    return hasBob ? bobContacts : repertoireContacts.filter(c => !c.aSurBob);
  };

  // État de synchronisation simulé
  const syncStatus = {
    state: loading ? 'loading' : (error ? 'error' : (contacts.length === 0 ? 'empty' : 'ready')),
    message: loading ? 'Synchronisation...' : (error || 'Synchronisé'),
    lastSync: Date.now()
  };
  ...
  const [stats, setStats] = useState({ total: 0, bobUsers: 0, nonBobUsers: 0, groupes: 0 });
  
  useEffect(() => {
    const calculateStats = async () => {
      try {
        const contactStats = await getStats();
        setStats({
          total: contactStats.mesContacts || 0,
          bobUsers: contactStats.contactsAvecBob || 0,
          nonBobUsers: contactStats.contactsSansBob || 0,
          groupes: 0 // À implémenter si nécessaire
        });
      } catch (error) {
  ...
  if (syncStatus.state === 'empty' || (!isLoading && contacts.length === 0)) {
    return (
      <View style={[styles.container, WebStyles.container]}>
        <EmptyStateWeb onRefresh={refreshData} isLoading={loading} />
      </View>
    );
  }
