...
export const ContactsGroupesView: React.FC<ContactsGroupesViewProps> = ({ 
  ...
  const { 
    repertoireContacts: contacts, 
    loading, 
    // Note: Les fonctions groupes ne sont pas encore implémentées dans la nouvelle API
    // Valeurs temporaires en attendant l'implémentation
  } = useContacts();

  // Simulation des groupes pour maintenir la compatibilité
  const groupes: any[] = [];
  const createGroupe = async (data: any) => {
    console.log('TODO: Implémenter createGroupe dans nouvelle API', data);
  };
  const updateGroupe = async (id: any, data: any) => {
    console.log('TODO: Implémenter updateGroupe dans nouvelle API', id, data);
  };
  const deleteGroupe = async (id: any) => {
    console.log('TODO: Implémenter deleteGroupe dans nouvelle API', id);
  };
